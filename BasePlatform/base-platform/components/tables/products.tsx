...
export function ProductsTable({ rows }: { rows: Product[]}) {
  ...
  return (
    <BaseTable colHeader={colHeader} action={actionButton} >
      {rows.map((item, key) => (
        <tr key={key}>
          <td className={`${Column()} w-full max-w-[30%]`}>{item.name}</td> 
          <td className={`${Column()} w-full max-w-[50%]`}>{item.description}</td> 
          <td className={`${Column()} w-full`}>€{item.price.amount / 100}</td>
          <td className={`${Column()} flex flex-row gap-4 w-fit`}>
      ...
      {rows.length === 0 && (
        <tr>
          <td className={`${Column()} text-center`} colSpan={5}>No products found</td>
        </tr>
      )}  
    </BaseTable>
  )
